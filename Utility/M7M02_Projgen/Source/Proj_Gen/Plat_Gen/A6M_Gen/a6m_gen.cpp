...
******************************************************************************/
std::unique_ptr<std::vector<ptr_t>> A6M_Gen::Pgt_Gen(std::vector<std::unique_ptr<class Mem_Info>>& List,
                                                     ptr_t& Total_Static)
{
    ...
    while(!Info->empty())
    {
        ...
        RBAR=Base|A6M_RBAR_VALID|Total_Static;
        RASR=A6M_RASR_ENABLE;
    ...
    {
        Pgt->push_back(A6M_RBAR_VALID|Count);
        Pgt->push_back(0);
    }
...
void A6M_Gen::Kernel_Conf_Hdr(std::unique_ptr<std::vector<std::string>>& List)
{
    ...
    Proj_Gen::Macro_Hex(List, "RME_A6M_INIT_ENTRY",PRC_DESC_ALIGN(this->Proj->Monitor->Code_Base)|0x01, MACRO_REPLACE);
    /* Init process's first thread's stack address */
    Proj_Gen::Macro_Hex(List, "RME_A6M_INIT_STACK",
    ...
    Proj_Gen::Macro_String(List, "RME_A6M_SYSTICK_VAL", this->Proj->Chip->Config["Systick_Value"], MACRO_REPLACE);

    /* Fixed attributes - we will refill these with database values */
    /* Number of MPU regions available */
    Proj_Gen::Macro_Int(List, "RME_A6M_REGION_NUM", this->Chip->Region, MACRO_REPLACE);
...
{
    /* Init process's first thread's entry point address */
    Proj_Gen::Macro_Hex(List, "RVM_A6M_INIT_ENTRY",PRC_DESC_ALIGN(this->Proj->Monitor->Code_Base)|0x01, MACRO_REPLACE);
    /* Init process's first thread's stack address */
    Proj_Gen::Macro_Hex(List, "RVM_A6M_INIT_STACK",
                        this->Proj->Monitor->Init_Stack_Base+this->Proj->Monitor->Init_Stack_Size-16, MACRO_REPLACE);
    /* Fixed attributes - we will refill these with database values */
    /* Number of MPU regions available */
    Proj_Gen::Macro_Int(List, "RVM_A6M_REGION_NUM", this->Chip->Region, MACRO_REPLACE);
