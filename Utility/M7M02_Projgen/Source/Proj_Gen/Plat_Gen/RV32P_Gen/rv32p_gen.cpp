...
Description : Generator for the RV32P platform.
...
ptr_t RV32P_Gen::Mem_Align(ptr_t Base, ptr_t Size, ptr_t Align_Order)
{
    ...
    try
    {
        ...
        if(Base==MEM_AUTO)
        {
             ...
             * too many pieces; this might cause issues on systems where the number of PMP
             * regions is scarce, but we can use manual alignment for these chips. */
...
Description : Recursively construct the page table for the RV32P port.
...
/* Function:RV32P_Gen::Pgt_Gen ************************************************
Description : Construct the raw page table MPU cache for the RV32P port.
Input       : std::vector<std::unique_ptr<class Mem_Info>>& - The list containing
                                                              memory segments.
Output      : ptr_t Total_Static - The total number of static regions used.
...
void RV32P_Gen::Kernel_Conf_Hdr(std::unique_ptr<std::vector<std::string>>& List)
{
    ...
    if (Proj->Buildsystem=="Makefile")
        Proj_Gen::Macro_Hex(List, "RME_RV32P_INIT_ENTRY",PRC_DESC_ALIGN(this->Proj->Monitor->Code_Base), MACRO_REPLACE);
    else
        Proj_Gen::Macro_Hex(List, "RME_RV32P_INIT_ENTRY",PRC_DESC_ALIGN(this->Proj->Monitor->Code_Base+8*4), MACRO_REPLACE);
    /* Init process's first thread's stack address */
    Proj_Gen::Macro_Hex(List, "RME_RV32P_INIT_STACK",
    ...
    Proj_Gen::Macro_String(List, "RME_RV32P_OSTIM_VAL", this->Proj->Chip->Config["Ostim_Value"]+"U", MACRO_REPLACE);

    /* Fixed attributes - we will refill these with database values */
    /* Number of MPU regions available */
    Proj_Gen::Macro_Int(List, "RME_RV32P_REGION_NUM", this->Chip->Region, MACRO_REPLACE);
}
...
void RV32P_Gen::Monitor_Conf_Hdr(std::unique_ptr<std::vector<std::string>>& List)
{
    /* Init process's first thread's entry point address */
    if(Proj->Buildsystem == "Makefile")
        Proj_Gen::Macro_Hex(List, "RVM_RV32P_INIT_ENTRY",PRC_DESC_ALIGN(this->Proj->Monitor->Code_Base), MACRO_REPLACE);
    else
        Proj_Gen::Macro_Hex(List, "RVM_RV32P_INIT_ENTRY",PRC_DESC_ALIGN(this->Proj->Monitor->Code_Base+8*4), MACRO_REPLACE);
    /* Init process's first thread's stack address */
    Proj_Gen::Macro_Hex(List, "RVM_RV32P_INIT_STACK",
                        this->Proj->Monitor->Init_Stack_Base+this->Proj->Monitor->Init_Stack_Size-16, MACRO_REPLACE);
    /* Fixed attributes - we will refill these with database values */
    /* Number of MPU regions available */
    Proj_Gen::Macro_Int(List, "RVM_RV32P_REGION_NUM", this->Chip->Region, MACRO_REPLACE);
}
