...
/* void */ Port::Port(xml_node_t* Root)
{
    ...
    try
    {
        ...
        if((XML_Child(Node,"Name",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0900: Name section is missing.");
        if(Temp->XML_Val_Len==0)
            throw std::invalid_argument("P0901: Name section is empty.");
        this->Name=std::make_unique<std::string>(Temp->XML_Val,(int)Temp->XML_Val_Len);

        /* Proc_Name */
        if((XML_Child(Node,"Process",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0902: Process name section is missing.");
        if(Temp->XML_Val_Len==0)
            throw std::invalid_argument("P0903: Process name section is empty.");
