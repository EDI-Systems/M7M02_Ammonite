...
/* void */ Inv::Inv(xml_node_t* Node)
{
    ...
    try
    {
        ...
        if((XML_Child(Node,"Name",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0800: Name section is missing.");
        if(Temp->XML_Val_Len==0)
            throw std::invalid_argument("P0801: Name section is empty.");
        this->Name=std::make_unique<std::string>(Temp->XML_Val,(int)Temp->XML_Val_Len);

        /* Stack Size */
        if((XML_Child(Node,"Stack_Size",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0802: Stack size section is missing.");
        if(XML_Get_Hex(Temp,&(this->Stack_Size))<0)
            throw std::invalid_argument("P0803: Stack size is not a valid hex integer.");
        if(this->Stack_Size==0)
            throw std::invalid_argument("P0804: Stack size cannot be zero.");
