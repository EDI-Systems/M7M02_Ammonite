...
/* void */ Vect::Vect(xml_node_t* Root)
{
    ...
    try
    {
        ...
        if((XML_Child(Node,"Name",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P1200: Name section is missing.");
        if(Temp->XML_Val_Len==0)
            throw std::invalid_argument("P1201: Name section is empty.");
        this->Name=std::make_unique<std::string>(Temp->XML_Val,(int)Temp->XML_Val_Len);

        /* Number */
        if((XML_Child(Node,"Number",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P1202: Number section is missing.");
        if(XML_Get_Uint(Temp,&(this->Num))<0)
            throw std::invalid_argument("P1203: Number is not a valid unsigned integer.");
    }
