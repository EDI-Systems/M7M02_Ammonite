...
/* Begin Function:Para::Add ***************************************************
Description : Add a single line to a paragraph. This is used exclusively for 
              printing reports.
Input       : std::unique_ptr<std::string>& Left - The left hand side contents.
              const s8_t* Right - The right hand side contents.
Output      : None.
Return      : None.
******************************************************************************/
void Para::Add(std::unique_ptr<std::string>& Left, const s8_t* Right, ...)
{
    s8_t* Buf;
    va_list Args;
    ptr_t Count;
    ...
    va_start(Args, Right);
    vsprintf(Buf, Right, Args);
    va_end(Args);

    Spaces=strlen(Left->c_str())+strlen(Buf);
    if(Spaces>79)
        Spaces=1;
    else
        Spaces=79-Spaces;

    Str=std::make_unique<std::string>(Left->c_str());
    for(Count=0;Count<Spaces;Count++)
        *Str+=" ";
    *Str+=Buf;

    this->Line.push_back(std::move(Str));
                      ...
                      std::unique_ptr<std::string>& Return)
{
    ...
    /* Print description */
    Line=std::make_unique<std::string>("Description : ");
    *Line+=*Desc;
    this->Add(std::move(Line));
...
void Para::Cdef(s8_t* Macro, s8_t* Value)
{
    ...
    /* Print to file */
    sprintf(Format, "#define %%-%ds    (%%s)", MACRO_ALIGN-4-8);
...
void Para::Cdef(s8_t* Macro, ret_t Value)
{
    ...
    /* Print to file */
    sprintf(Format, "#define %%-%lds    (%%lld)", MACRO_ALIGN-4-8);
    sprintf(Buf, Format, Macro, Value);
...
void Para::Cdef(s8_t* Macro, ptr_t Value)
{
    ...
    /* Print to file */
    sprintf(Format, "#define %%-%lds    (0x%%llX)", MACRO_ALIGN-4-8);
    sprintf(Buf, Format, Macro, Value);
...
/* Begin Function:Doc::Date ***************************************************
Description : Get the current date.
Input       : None.
Output      : None.
Return      : std::unique_ptr<std::string> - The current time.
******************************************************************************/
std::unique_ptr<std::string> Doc::Date(void)
{
    s8_t Date[64];
    time_t Time;
    ...
    return std::make_unique<std::string>(Date);
}
/* End Function:Doc::Date ****************************************************/
