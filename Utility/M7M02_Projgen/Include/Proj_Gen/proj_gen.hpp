...
public:
    ...
    /* void */ Proj_Gen(const std::string& Name,
    ...
    static void Macro_Mem(const std::string& Prefix,
                          std::unique_ptr<std::vector<std::string>>& List,
                          const class Mem_Info* Mem);

    void Boot_Hdr_Mem(const std::string& Prefix,
                      std::unique_ptr<std::vector<std::string>>& List);
    ...
    void Process_Nat_Src(class Native* Nat);
