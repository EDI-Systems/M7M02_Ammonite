...
/* void */ Proj::Proj(xml_node_t* Node)
{
    ...
    try
    {
        ...
        if((Node->XML_Tag_Len!=7)||(strncmp(Node->XML_Tag,"Project",7)!=0))
            throw std::invalid_argument("P0001: Project XML is malformed.");

        /* Name */
        if((XML_Child(Node,"Name",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0002: Name section is missing.");
        if(Temp->XML_Val_Len==0)
            throw std::invalid_argument("P0003: Name section is empty.");
        this->Name=std::make_unique<std::string>(Temp->XML_Val,(int)Temp->XML_Val_Len);

        /* Platform */
        if((XML_Child(Node,"Platform",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0004: Platform section is missing.");
        if(Temp->XML_Val_Len==0)
            throw std::invalid_argument("P0005: Platform section is empty.");
        ...
        if((XML_Child(Node,"Chip_Class",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0006: Chip class section is missing.");
        if(Temp->XML_Val_Len==0)
            throw std::invalid_argument("P0007: Chip class section is empty.");
        this->Chip_Class=std::make_unique<std::string>(Temp->XML_Val,(int)Temp->XML_Val_Len);

        /* Chip_Full */
        if((XML_Child(Node,"Chip_Full",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0008: Chip fullname section is missing.");
        if(Temp->XML_Val_Len==0)
            throw std::invalid_argument("P0009: Chip fullname section is empty.");
        this->Chip_Full=std::make_unique<std::string>(Temp->XML_Val,(int)Temp->XML_Val_Len);

        /* RME */
        if((XML_Child(Node,"RME",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0010: RME section is missing.");
        this->RME=std::make_unique<class RME>(Temp);

        /* RVM */
        if((XML_Child(Node,"RVM",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0011: RVM section is missing.");
        this->RVM=std::make_unique<class RVM>(Temp);

        /* Extra memory segments - not necesssarily exist */
        if((XML_Child(Node,"Extmem",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0012: Extra memory section is missing.");
        if(XML_Child(Temp,0,&Trunk)<0)
            throw std::invalid_argument("P0013: Extra memory section parsing internal error.");
        while(Trunk!=0)
        {
            if((XML_Child(Trunk,"Type",&Mem_Type)<0)||(Mem_Type==0))
                throw std::invalid_argument("P0014: Extra memory type section is missing.");
            if(Mem_Type->XML_Val_Len==0)
                throw std::invalid_argument("P0015: Extra memory type section is empty.");
            
            Str=std::make_unique<std::string>(Mem_Type->XML_Val,(int)Mem_Type->XML_Val_Len);

            if(*Str=="Code")
                this->Extmem_Code.push_back(std::make_unique<class Mem>(Trunk));
            else if(*Str=="Data")
                this->Extmem_Data.push_back(std::make_unique<class Mem>(Trunk));
            else if(*Str=="Device")
                this->Extmem_Device.push_back(std::make_unique<class Mem>(Trunk));
            else
                throw std::invalid_argument("P0016: Extra memory type is malformed.");
        ...
        if((XML_Child(Node,"Shmem",&Temp)<0)||(Temp==0))
            throw std::invalid_argument("P0017: Shared memory section is missing.");
        if(XML_Child(Temp,0,&Trunk)<0)
            throw std::invalid_argument("P0018: Shared memory section parsing internal error.");
        while(Trunk!=0)
        {
            if((XML_Child(Trunk,"Type",&Mem_Type)<0)||(Mem_Type==0))
                throw std::invalid_argument("P0019: Shared memory type section is missing.");
            if(Mem_Type->XML_Val_Len==0)
                throw std::invalid_argument("P0020: Shared memory type section is empty.");

            Mem_Block=std::make_unique<class Mem>(Trunk);
            if(Mem_Block->Name==nullptr)
                throw std::invalid_argument("P0021: Shared memory name cannot be omitted.");
            Mem_Block->Is_Shared=1;

            Str=std::make_unique<std::string>(Mem_Type->XML_Val,(int)Mem_Type->XML_Val_Len);

            if(*Str=="Code")
                this->Shmem_Code.push_back(std::move(Mem_Block));
            else if(*Str=="Data")
                this->Shmem_Data.push_back(std::move(Mem_Block));
            else if(*Str=="Device")
                this->Shmem_Device.push_back(std::move(Mem_Block));
            else
                throw std::invalid_argument("P0022: Shared memory type is malformed.");

            if(XML_Child(Temp,"",&Trunk)<0)
                throw std::invalid_argument("P0018: Shared memory section parsing internal error.");
        }

        /* Process - Not necessarily exist */
        if((XML_Child(Node,"Process",&Temp)>=0)&&(Temp!=0))
        {
            if(XML_Child(Temp,0,&Trunk)<0)
                throw std::invalid_argument("P0023: Process section parsing internal error.");
            while(Trunk!=0)
            {
        ...
        /* Virtual machine - not necessarily exist */
        if((XML_Child(Node,"Virtual",&Temp)>=0)&&(Temp!=0))
        {
            if(XML_Child(Temp,0,&Trunk)<0)
                throw std::invalid_argument("P0024: Virtual machine section parsing internal error.");
            while(Trunk!=0)
            {
                this->Proc.push_back(std::make_unique<class Virt>(Trunk));
                /* Place this into virtual machine list as well */
                this->Virt.push_back(static_cast<class Virt*>(this->Proc.back().get()));
