...
template std::string* Kobj::Check_Kobj<class Kern>(std::vector<std::unique_ptr<class Kern>>& List);
/* Actually, this is not a kernel object but also suffices the template. This is for shared memory blocks */
template std::string* Kobj::Check_Kobj<class Mem>(std::vector<std::unique_ptr<class Mem>>& List);
template std::string* Kobj::Check_Kobj<class Shmem>(std::vector<std::unique_ptr<class Shmem>>& List);
