...
Input       : rvm_ptr_t Vect_Num - The physical vector number's place in the kernel function table.
...
Input       : rvm_cnt_t Int_ID - The interrupt registration ID returned by RVM_Reg_Int.
Output      : None.
Return      : rvm_ret_t - If successful, 0; else an error code.
******************************************************************************/
rvm_ret_t RVM_Del_Int(rvm_cnt_t Int_ID)
{
...
Input       : rvm_ptr_t Int_Num - The interrupt number for this channel.
              rvm_ptr_t VMID - The sender's VMID. Only the VM with this ID is
                               allowed to send to the channel.
Output      : None.
Return      : rvm_ret_t - If successful, 0; else an error code.
******************************************************************************/
rvm_ret_t RVM_Reg_Evt(rvm_ptr_t Int_Num, rvm_ptr_t VMID)
{
    return RVM_Hypercall(RVM_HYP_REGEVT,Int_Num,VMID,0,0);
}
/* End Function:RVM_Reg_Evt **************************************************/

/* Begin Function:RVM_Del_Evt *************************************************
Description : Delete an event channel previously created.
Input       : rvm_ptr_t Evt_ID - The event channel to delete. The event channel 
                                 must be created by this VM.
Output      : None.
Return      : rvm_ret_t - If successful, 0; else an error code.
******************************************************************************/
rvm_ret_t RVM_Del_Evt(rvm_cnt_t Evt_ID)
{
...
Input       : rvm_ptr_t Evt_ID - The event channel to send to.
Output      : None.
Return      : rvm_ret_t - If successful, 0; else an error code.
******************************************************************************/
rvm_ret_t RVM_Send_Evt(rvm_ptr_t Evt_ID)
{
...
Input       : None.
Output      : None.
Return      : rvm_ret_t - If successful, the channel ID; else an error code.
******************************************************************************/
rvm_ret_t RVM_Query_Evt(rvm_ptr_t VMID)
{
...
Return      : rvm_ret_t - If successful, 0; else an error code.
******************************************************************************/
rvm_ret_t RVM_Query_VM(char* Name)
{
    rvm_cnt_t Count;
    char* Name_Array;
...
Description : See if there is a VM with this name. If there is, return its VM ID.
Input       : rvm_ptr_t Period - The new period of timer interrupts.
Output      : None.
Return      : rvm_ret_t - If successful, 0; else an error code.
******************************************************************************/
rvm_ret_t RVM_Tim_Prog(rvm_ptr_t Period)
{
